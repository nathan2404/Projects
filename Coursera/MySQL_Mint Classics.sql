...
(
	...
	Count(Case when warehousecode =  'A' then productcode End) as 'A', 
    Count(Case when warehousecode =  'B' then productcode End) as 'B', 
    Count(Case when warehousecode =  'C' then productcode End) as 'C', 
    Count(Case when warehousecode =  'D' then productcode End) as 'D'
    From Piv_Product_Line
...
Select customernumber, ordernumber , orderdate , Next_Orderdate, 
	DATEDIFF(Next_Orderdate, orderdate) as 'Days_Difference'
...
Select customerNumber,
		Round(AVG(Days_Difference),1) as 'AVG_Ordering_In_Days'
From Day_Diff
...
(
	Select customernumber, ordernumber , orderdate, Next_Orderdate,
    extract(Year_Month From Next_Orderdate) as 'Year_and_Month_Futu',
    extract(Year_Month From Orderdate) as 'Year_and_Month_Curr'
	From Nxt_Orddate
...
(
	Select customernumber, ordernumber , orderdate, Next_Orderdate, 
	DATEDIFF(Next_Orderdate, orderdate) as 'Days_Difference',
	period_diff(Year_and_Month_Futu, Year_and_Month_Curr ) as 'Months_Difference'
	From Conversion_YR_Month
